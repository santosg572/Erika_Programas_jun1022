...
SegmentarGris <- function(i=0, j=0){
	...
	for (ii in (i-1):(i+1)){
		for (jj in (j-1):(j+1)){	
			if (1 <= ii & ii <= NX & 1 <=jj & jj <= NY){
				if (abs(IMGT[ii,jj]-MED) < SD1 & IMGT[ii,jj] < 999 & N>0){
#					cat(ii, jj, IMGT[ii,jj], MED, '\n')
					N <<- N-1
					SegmentarGris(ii, jj)
				}
		    }
...
toma_Muestra <- function(img=0, i0=0, j0=0, del=0, val=0){
	imgt = img
	imgt[(i0-del):(i0+del), (j0-del):(j0+del)] = val
